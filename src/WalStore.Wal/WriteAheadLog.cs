...
public sealed class WriteAheadLog : IWalLogger
{
    ...
    public async Task WriteRecordAsync(byte[] data, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                await _segmentManager.RotateAsync(_config.ForceSyncEnabled, _config.MaxSegments);
            }
    ...
    public async Task<List<WalRecord>> ReadAllRecordsAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            return await ReadCurrentSegmentRecordsAsync(ct);
        }
        finally
        {
            _lock.Release();
    ...
    /// <summary>
    /// Reads the current segment without taking <see cref="_lock"/>, so callers that
    /// already hold it cannot self-deadlock on the non-reentrant semaphore.
    /// </summary>
    private async Task<List<WalRecord>> ReadCurrentSegmentRecordsAsync(CancellationToken ct)
    {
        var segment = _segmentManager.CurrentSegment;
        await segment.FlushAsync(ct);
        return await ReadRecordsFromFileAsync(segment.FilePath, ct);
    }

    private async Task<List<WalRecord>> ReadRecordsFromFileAsync(string filePath, CancellationToken ct)
    {
    ...
    /// <summary>
    /// Highest LSN on disk. Falls back to older segments when the current one is empty —
    /// a fresh segment after rotation, or one that was emptied during repair — so that an
    /// LSN is never handed out twice.
    /// </summary>
    private async Task<ulong> DiscoverLastLsnAsync()
    {
        var records = await ReadCurrentSegmentRecordsAsync(CancellationToken.None);
        if (records.Count > 0)
            return records[^1].LogSequenceNumber;

        var currentPath = _segmentManager.CurrentSegment.FilePath;
        var segmentFiles = WalSegmentNaming.GetOrderedSegmentFiles(_config.Directory);

        for (var i = segmentFiles.Length - 1; i >= 0; i--)
        {
            if (string.Equals(segmentFiles[i], currentPath, StringComparison.OrdinalIgnoreCase))
                continue;

            var older = await ReadRecordsFromFileAsync(segmentFiles[i], CancellationToken.None);
            if (older.Count > 0)
                return older[^1].LogSequenceNumber;
        }
